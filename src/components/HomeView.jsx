...
const HomeView = ({
  ...
  const weekProgress = calculateWeekProgress(effectiveWeek, runningData, cyclingData, weightData);
  ...
  // Find data index for running/cycling workouts
  ...
  const getRecentActivity = () => {
    ...
    // Sort by week (descending) and return last 5
    return allWorkouts
      .sort((a, b) => b.week - a.week || (b.day > a.day ? 1 : -1))
      .slice(0, 5);
  };
  ...
  const WorkoutCard = ({ workout, idx }) => {
    ...
    }, [debouncedDuration, dataIndex, workoutData, activityType, updateRunning]);
      ...
      >
        ...
        <div className="flex items-center justify-between flex-wrap gap-3 mb-3">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-slate-300 break-words">{workout.detail}</p>
        ...
        {/* Quick Entry Fields for Running/Cycling */}
        {workout.status !== 'completed' && workout.status !== 'skipped' && 
         (workout.type === 'Running' || workout.type === 'Cycling') && workoutData && (
          <div className="mt-4 pt-4 border-t border-slate-600/50">
            <div className={`grid gap-3 ${workout.type === 'Running' ? 'grid-cols-2 sm:grid-cols-4' : 'grid-cols-2 sm:grid-cols-3'}`}>
              {/* Duration (Running only) */}
              {workout.type === 'Running' && (
                <div>
